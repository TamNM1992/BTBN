using AutoMapper;
using BaoTangBn.Common;
using BaoTangBn.Common.Extensions;
using BaoTangBn.Data.Dtos;
using BaoTangBn.Data.Models;
using BaoTangBn.Repo.SuKienSapDienRaRepo;
using Microsoft.Extensions.Options;

using System;
using System.Collections.Generic;



namespace BaoTangBn.Service.SuKienSapDienRaService
{

    public class SuKienSapDienRaService : ISuKienSapDienRaService
    {
        private ISuKienSapDienRaRepository _repo;
        private readonly IMapper _mapper;
        private readonly AppSettings _appSettings;
        public SuKienSapDienRaService(ISuKienSapDienRaRepository repo ,IMapper mapper, IOptions<AppSettings> appSettings)
        {
            _repo = repo;
            _mapper = mapper;
            _appSettings = appSettings.Value;
        }

        public IEnumerable<SuKienSapDienRa_ShowOnViewer> GetList(bool admin)
        {
            List<SuKienSapDienRa_ShowOnViewer> temp1 = new List<SuKienSapDienRa_ShowOnViewer>();
            var temp2 = _repo.GetAll();
            var temp3 = temp2.ToList();

            temp3.RemoveAll(x => x.DaXoa == true);

            if (admin == false)
            {
                temp3.RemoveAll(x => x.TrangThaiXuatBan == false);
            }

            for (int i = 0; i < temp3.Count; i++)
            {
                temp1.Add(_mapper.Map<SuKienSapDienRa, SuKienSapDienRa_ShowOnViewer>(temp3[i]));
            }
            return temp1;
        }
        public IEnumerable<SuKienSapDienRa_ShowOnUser> SearchByTenAndTieuDe(string keyWord)
        {
            List<SuKienSapDienRa_ShowOnUser> temp1 = new List<SuKienSapDienRa_ShowOnUser>();
            var temp2 = _repo.GetAll();
            var temp3 = temp2.ToList();

            temp3.RemoveAll(x => x.DaXoa == true);
            for (int i = 0; i < temp3.Count; i++)
            {
                if (temp3[i].Ten.Contains(keyWord) == true || temp3[i].TieuDe.Contains(keyWord) == true)
                {
                    temp1.Add (_mapper.Map<SuKienSapDienRa, SuKienSapDienRa_ShowOnUser>(temp3[i]));
                }
            }
            return temp1;
        }
        public IEnumerable<SuKienSapDienRa_Related> GetRelated(Guid IDBaiViet, int pre_count, int next_count)
        {
            SuKienSapDienRa[] array = new SuKienSapDienRa[pre_count + next_count];
            var temp = _repo.GetRelated();
            temp.SortByField("asc", "NgayTao");
            SuKienSapDienRa[] arraytemp = temp.ToArray();
            int i;
            int j;
            int k;
            for (i = 0; i < arraytemp.Length; i++)
            {
                if (arraytemp[i].ID == IDBaiViet)
                    break;
            }
            k = i;
            for (j = 0; j < pre_count; j++)
            {
                if (k - 1 <0)
                    break;
                array[j] = arraytemp[k-1];
                k--;
                
            }
            k = i;
            for (j = pre_count; j < array.Length; j++)
            {
                if ( k + 1>= arraytemp.Length)
                    break;
                array[j] = arraytemp[k + 1];
                k++;
                
            }
            SuKienSapDienRa_Related[] relate = new SuKienSapDienRa_Related[pre_count + next_count];
            for ( i = 0; i< array.Length; i++)
            {
                relate[i] = _mapper.Map<SuKienSapDienRa, SuKienSapDienRa_Related>(array[i]);
            }
            relate.ToList();
            
            return relate;

        }
        public bool AddSuKienSapDienRa(SuKienSapDienRaDto SuKienSapDienRaDto, string token)
        {
            var _userID = General.GetIDInToken(token);
            var SuKienSapDienRaEntity = _mapper.Map<SuKienSapDienRaDto, SuKienSapDienRa>(SuKienSapDienRaDto);

            SuKienSapDienRaEntity.IDNguoiTao = _userID;

            SuKienSapDienRaEntity.NgayTao = DateTime.UtcNow;
            var temp = _repo.AddSuKienSapDienRa(SuKienSapDienRaEntity);
            return temp;
        }
        public bool XoaSuKienSapDienRa(Guid ID_BaiCanXoa, string token)
        {
            var ID_NguoiXoa = General.GetIDInToken(token);


            var temp = _repo.XoaSuKienSapDienRa(ID_BaiCanXoa, ID_NguoiXoa);
            return temp;
        }
        public bool EditSuKienSapDienRa(Guid IDBaiCanSua, string token, SuKienSapDienRaDto SuKienSapDienRaDto)
        {
            var IDNguoiSua = General.GetIDInToken(token);

            var temp = _repo.EditSuKienSapDienRa(IDBaiCanSua,IDNguoiSua, SuKienSapDienRaDto);
            return temp;
        }
        public SuKienSapDienRa_Detail ShowDetails(Guid id)
        {
            return _repo.ShowDetails(id);
        }

    }
}