...
namespace BaoTangBn.API.Controllers
{
    ...
    [ApiController]
    [Authorize]
    public class BaoVatQuocGia_AdminController : ControllerBase
    {
        private IBaoVatQuocGiaService _BaoVatQuocGiaService;
        public BaoVatQuocGia_AdminController(IBaoVatQuocGiaService BaoVatQuocGiaService)
        {
            _BaoVatQuocGiaService = BaoVatQuocGiaService;
        }
        ...
        public IActionResult GetListAndPaging(FilterBase filter)
        {
            ...
            try
            {
                var temp = _BaoVatQuocGiaService.GetList(true).ToList();
        ...
        [Route("Add")]
        public IActionResult AddBaoVatQuocGia([FromBody] BaoVatQuocGiaDto BaoVatQuocGiaDto)
        {
            ResponseBase response = new ResponseBase();
            try
            {
                var token = HttpContext.Request.Headers["Authorization"].FirstOrDefault()?.Split(" ").Last();
                var temp = _BaoVatQuocGiaService.AddBaoVatQuocGia(BaoVatQuocGiaDto, token);
                if (temp == false)
                {
        ...
        [Role(Common.Enums.Role.Boss)]
        [Route("Delete")]
        public IActionResult DeleteBaoVatQuocGia(Guid IdBaiCanXoa)
        {
            ResponseBase response = new ResponseBase();
            try
            {
                var token = HttpContext.Request.Headers["Authorization"].FirstOrDefault()?.Split(" ").Last();
                var temp = _BaoVatQuocGiaService.XoaBaoVatQuocGia(IdBaiCanXoa, token);
                if (temp == false)
                {
                    response.Code = ErrorCodeMessage.OperationFail.Key;
                    response.Message = ErrorCodeMessage.OperationFail.Value;
        ...
        public IActionResult SearchByTenAndTieuDe(string keyWord, FilterBase filter)
        {
            ...
            try
            {
                var temp = _BaoVatQuocGiaService.SearchByTenAndTieuDe(keyWord).ToList();
                if (temp != null)
                {
                    if (filter.SortField == null)
                    {
        ...
        [Role(Common.Enums.Role.Boss)]
        [Route("Edit")]
        public IActionResult EditBaoVatQuocGia(Guid IdBaiCanSua, [FromBody] BaoVatQuocGiaDto BaoVatQuocGiaDto)
        {
            ResponseBase response = new ResponseBase();
            try
            {
                var token = HttpContext.Request.Headers["Authorization"].FirstOrDefault()?.Split(" ").Last();
                var temp = _BaoVatQuocGiaService.EditBaoVatQuocGia(IdBaiCanSua, token, BaoVatQuocGiaDto);
                if (temp == false)
                {
                    response.Code = ErrorCodeMessage.OperationFail.Key;
                    response.Message = ErrorCodeMessage.OperationFail.Value;
