...
namespace BaoTangBn.API.Controllers
{
    ...
    [ApiController]
    [Authorize]
    public class Video_AdminController : ControllerBase
    {
        private IVideoService _VideoService;
        public Video_AdminController(IVideoService VideoService)
        {
        ...
        public IActionResult GetListAndPaging(FilterBase filter)
        {
            ...
            try
            {
                var temp = _VideoService.GetList(true).ToList();
        ...
        public IActionResult AddVideo([FromBody] VideoDto VideoDto)
        {
            ...
            try
            {
                ...
                var temp = _VideoService.AddVideo(VideoDto, token);
                if (temp == false)
                {
        ...
        [Role(Common.Enums.Role.Boss)]
        [Route("Delete")]
        public IActionResult DeleteVideo(Guid IdBaiCanXoa)
        {
            ResponseBase response = new ResponseBase();
            try
            {
                var token = HttpContext.Request.Headers["Authorization"].FirstOrDefault()?.Split(" ").Last();
                var temp = _VideoService.XoaVideo(IdBaiCanXoa, token);
                if (temp == false)
                {
                    response.Code = ErrorCodeMessage.OperationFail.Key;
                    response.Message = ErrorCodeMessage.OperationFail.Value;
        ...
        [Role(Common.Enums.Role.Boss)]
        [Route("Edit")]
        public IActionResult EditVideo(Guid IdBaiCanSua, [FromBody] VideoDto VideoDto)
        {
            ResponseBase response = new ResponseBase();
            try
            {
                var token = HttpContext.Request.Headers["Authorization"].FirstOrDefault()?.Split(" ").Last();
                var temp = _VideoService.EditVideo(IdBaiCanSua, token, VideoDto);
                if (temp == false)
                {
                    response.Code = ErrorCodeMessage.OperationFail.Key;
                    response.Message = ErrorCodeMessage.OperationFail.Value;
