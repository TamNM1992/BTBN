...
namespace BaoTangBn.API.Controllers
{
    ...
    [ApiController]
    [Authorize]
    public class CongTacSuuTam_AdminController : ControllerBase
    {
        private ICongTacSuuTamService _CongTacSuuTamService;
        public CongTacSuuTam_AdminController(ICongTacSuuTamService CongTacSuuTamService)
        {
        ...
        public IActionResult GetListAndPaging(FilterBase filter)
        {
            ...
            try
            {
                var temp = _CongTacSuuTamService.GetList(true).ToList();
        ...
        [Route("Add")]
        public IActionResult AddCongTacSuuTam([FromBody] CongTacSuuTamDto CongTacSuuTamDto)
        {
            ResponseBase response = new ResponseBase();
            try
            {
                var token = HttpContext.Request.Headers["Authorization"].FirstOrDefault()?.Split(" ").Last();
                var temp = _CongTacSuuTamService.AddCongTacSuuTam(CongTacSuuTamDto, token);
                if (temp == false)
                {
        ...
        [Role(Common.Enums.Role.Boss)]
        [Route("Delete")]
        public IActionResult DeleteCongTacSuuTam(Guid IdBaiCanXoa)
        {
            ResponseBase response = new ResponseBase();
            try
            {
                var token = HttpContext.Request.Headers["Authorization"].FirstOrDefault()?.Split(" ").Last();
                var temp = _CongTacSuuTamService.XoaCongTacSuuTam(IdBaiCanXoa, token);
                if (temp == false)
                {
                    response.Code = ErrorCodeMessage.OperationFail.Key;
                    response.Message = ErrorCodeMessage.OperationFail.Value;
        ...
        public IActionResult SearchByTenAndTieuDe(string keyWord, FilterBase filter)
        {
            ...
            try
            {
                var temp = _CongTacSuuTamService.SearchByTenAndTieuDe(keyWord).ToList();
                if (temp != null)
                {
                    if (filter.SortField == null)
                    {
        ...
        [Role(Common.Enums.Role.Boss)]
        [Route("Edit")]
        public IActionResult EditCongTacSuuTam(Guid IdBaiCanSua, [FromBody] CongTacSuuTamDto CongTacSuuTamDto)
        {
            ResponseBase response = new ResponseBase();
            try
            {
                var token = HttpContext.Request.Headers["Authorization"].FirstOrDefault()?.Split(" ").Last();
                var temp = _CongTacSuuTamService.EditCongTacSuuTam(IdBaiCanSua, token, CongTacSuuTamDto);
                if (temp == false)
                {
                    response.Code = ErrorCodeMessage.OperationFail.Key;
                    response.Message = ErrorCodeMessage.OperationFail.Value;
