...
namespace BaoTangBn.API.Controllers
{
    ...
    [ApiController]
    public class HuongDanThamQuan_ViewerController : ControllerBase
    {
        private IHuongDanThamQuanService _HuongDanThamQuanService;
        private readonly IMapper _mapper;
        public HuongDanThamQuan_ViewerController(IHuongDanThamQuanService HuongDanThamQuanService, IMapper mapper)
        {
        ...
        public IActionResult GetListAndPaging(FilterBase filter)
        {
            ...
            try
            {
                var temp = _HuongDanThamQuanService.GetList(false).ToList();
        ...
        public IActionResult GetRelated(Guid IDBaiViet, int pre_count, int next_count)
        {
            ...
            try
            {
                var temp = _HuongDanThamQuanService.GetRelated(IDBaiViet, pre_count, next_count).ToList();
        ...
        public IActionResult ShowDetails(Guid ID)
        {
            ...
            try
            {
                var temp = _HuongDanThamQuanService.ShowDetails(ID);
