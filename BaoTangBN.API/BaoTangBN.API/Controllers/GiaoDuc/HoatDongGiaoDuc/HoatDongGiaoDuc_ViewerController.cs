...
namespace BaoTangBn.API.Controllers
{
    ...
    [ApiController]
    public class HoatDongGiaoDuc_ViewerController : ControllerBase
    {
        private IHoatDongGiaoDucService _HoatDongGiaoDucService;
        private readonly IMapper _mapper;
        public HoatDongGiaoDuc_ViewerController(IHoatDongGiaoDucService HoatDongGiaoDucService, IMapper mapper)
        {
        ...
        public IActionResult GetListAndPaging(FilterBase filter)
        {
            ...
            try
            {
                var temp = _HoatDongGiaoDucService.GetList(false).ToList();
        ...
        public IActionResult GetRelated(Guid IDBaiViet, int pre_count, int next_count)
        {
            ...
            try
            {
                var temp = _HoatDongGiaoDucService.GetRelated(IDBaiViet, pre_count, next_count).ToList();
        ...
        public IActionResult ShowDetails(Guid ID)
        {
            ...
            try
            {
                var temp = _HoatDongGiaoDucService.ShowDetails(ID);
